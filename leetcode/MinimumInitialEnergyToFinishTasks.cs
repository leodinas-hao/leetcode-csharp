...
public class Solution
{
  ...
  {
    // sort task list by putting the task of which having smaller gap (minimum - actual) in front
    // sort with `delegate`
    // Array.Sort(tasks, delegate (int[] task1, int[] task2)
    // {
    //   return (task1[1] - task1[0]) - (task2[1] - task2[0]);
    // });

    // sort with lambda
    Array.Sort(tasks, (int[] task1, int[] task2) => (task1[1] - task1[0]) - (task2[1] - task2[0]));

    int minimum = 0;
    foreach (var task in tasks)
    {
      minimum = Math.Max(minimum, task[1] - task[0]) + task[0];
    }
    return minimum;
  }
}
