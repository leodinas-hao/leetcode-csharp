...
namespace LeetCode.FindTheStudentThatWillReplaceTheChalk;
...
public class Solution
{
  ...
  {
    int i = 0;
    // a simple `chalk.Sum()` will produce OverflowException when having large input
    // hence do the sum in old fashion way
    long sum = 0;
    foreach (var c in chalk) sum += c;
    k = (int)(k % sum);
