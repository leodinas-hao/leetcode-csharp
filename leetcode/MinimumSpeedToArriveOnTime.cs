...
namespace LeetCode.MinimumSpeedToArriveOnTime;

public class Solution
{
  // helper function to check if given speed could meet the hour limit
  private bool Helper(int[] dist, double hour, int speed)
  {
    double time = 0;
    for (int i = 0; i < dist.Length - 1; i++)
      time += Math.Ceiling((double)dist[i] / speed);

    // no need to wait for the fractional hours for the last train
    return time + (double)dist[^1] / speed <= hour;
  }
  ...
  public int MinSpeedOnTime(int[] dist, double hour)
  {
    ...
    // edge cases where not enough hours to reach to take on the last train
    // consider Example 3
