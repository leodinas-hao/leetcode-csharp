...
public class Bank
{
  ...
  private bool AccountExists(int account)
  {
    // account number should be 0 indexed number here
  ...
  public bool Transfer(int account1, int account2, long money)
  {
    account1 -= 1;
    account2 -= 1;
    if (AccountExists(account1) && AccountExists(account2) && Balance[account1] >= money)
    {
  ...
  {
    account -= 1;
    if (AccountExists(account) && Balance[account] >= money)
    {
      Balance[account] -= money;
      return true;
    }
