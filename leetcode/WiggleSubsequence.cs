...
namespace LeetCode.WiggleSubsequence;

public class Solution
{
  public int WiggleMaxLength(int[] nums)
  {
    // when only less than 1 element, just return the length
    // when there are 2 elements && 2 elements are the same in such case the wiggle length is 1
    ...
    for (int i = 2; i < nums.Length; i++)
    {
      ...
      // when it's wiggle (diff is opposite of preDiff)
