...
namespace LeetCode.FindTheSmallestDivisorGivenAThreshold;

public class Solution
{
  public int SmallestDivisor(int[] nums, int threshold)
  {
    var sum = (int divisor) => nums.Sum((num) => (int)Math.Ceiling(num / (double)divisor));
    ...
    {
      int mid = (lo + hi) / 2;
      if (sum(mid) > threshold) lo = mid + 1;
      else hi = mid;  // sum of division's <= threshold
    }
    return hi;
  }
}
