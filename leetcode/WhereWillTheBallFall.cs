...
namespace LeetCode.WhereWillTheBallFall;
...
{
  private int FindBallDropColumn(int[][] grid, int row, int col)
  {
    // already bottom row
    ...
    // move to next row
    return FindBallDropColumn(grid, row + 1, nextCol);
  }
  ...
  public int[] FindBall(int[][] grid)
  {
    ...
    for (int i = 0; i < grid[0].Length; i++)
      ans[i] = FindBallDropColumn(grid, 0, i);
    return ans;
  }
}
